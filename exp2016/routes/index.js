...
router.get(/^\/((\w+)\-*(\w*)\-*(\w*))$/, function(req,res){
  ...
  function Produkt(a,b,c,d,e,f,g){
    this.id = a;
    this.name = b;
    this.provider = c;
    this.logo = d;
    ...
    response.on('end',function(){
      ...
      parseString(xml, function(err,result){
        ...
        for(var i=0;i<products_name.length;i++){
            var p = new Produkt(i,products_name[i],products_provider[i],products_logo[i],products_prez[i],products_application[i],products_description[i]);
            if(i<11){p.category ="Karty-kredytowe"; }
            if(i>=11 && i<30){p.category ="Konta-osobiste"; }
            if(i>=30 && i<37){p.category ="Kredyty-mieszkaniowe"; }
            if(i>=37 && i<50){p.category ="Kredyty-gotowkowe"; }
            if(i>=50 && i<53){p.category ="Kredyty-samochodowe"; }
            if(i>=53 && i<56){p.category ="Konta-mlodziezowe"; }
            if(i>=56 && i<63){p.category ="Kredyty-konsolidacyjne"; }
            if(i>=63 && i<64){p.category ="Kredyty-odnawialne"; }
            if(i>=64 && i<66){p.category ="Pozyczki-spolecznosciowe"; }
            if(i>=66 && i<67){p.category ="Doradztwo-ds-nieruchomosci"; }
            if(i>=67 && i<68){p.category ="Ksiegarnia"; }
            if(i>=68 && i<69){p.category ="Zwrot-podatku"; }
            if(i>=69 && i<71){p.category ="Programy-niefinansowe"; }
            if(i>=71 && i<110){p.category ="a"; }
            if(i>=110 && i<111){p.category ="Informacja-kredytowa"; }
            if(i>=111 && i<113){p.category ="Fundusze-inwestycyjne"; }
            if(i>=113 && i<122){p.category ="Lokaty"; }
            if(i>=122 && i<128){p.category ="Rachunki-inwestycyjne"; }
            if(i>=128 && i<129){p.category ="IKE"; }
            if(i>=129 && i<130){p.category ="Inwestycja-alternatywna"; }
            if(i>=130 && i<131){p.category ="Waluty"; }
            if(i>=131 && i<135){p.category ="Ubezpieczenia-na-zycie"; }
            if(i>=135 && i<136){p.category ="Ubezpieczenia-majatkowe"; }
            if(i>=136 && i<140){p.category ="Ubezpieczenia-komunikacyjne"; }
            if(i>=140 && i<141){p.category ="Ubezpieczenia-turystyczne"; }
            if(i>=141 && i<143){p.category ="Leasing"; }
            if(i>=143 && i<153){p.category ="Rachunki-firmowe"; }
            if(i>=153 && i<162){p.category ="Kredyty-dla-firm"; }
            if(i>=162 && i<182){p.category ="e-Sklepy"; }
